...
HDF5_FILE = '/raid/nanopore/shubham/20190629_nanopore_data/raw_signal/raw_signal_3.hdf5'
CONV_INPUT_FILE = '/raid/nanopore/shubham/20190629_nanopore_data/oligo_files/conv_input_3.txt'
...
for _ in range(NUM_ATTEMPTS):
    ...
    if rc:
        rc_flag = '--rc'
    print(rc_flag)
    subprocess.run([PATH_TO_CPP_EXEC,'-m','decode','-i',new_post_filename,'-o',decoded_filename,'--msg-len',str(MSG_LEN),'-l',str(LIST_SIZE),'-t',str(NUM_THREADS),'--mem-conv',str(MEM_CONV),rc_flag,'--max-deviation','10','-r',str(RATE_CONV)])
